-- [[ 1. ЛОГИ В ДИСКОРД ]]
local success_log = pcall(function()
    local webhook_url = "https://discordapp.com/api/webhooks/1483494052937859124/8ARo2_JKrvhV0mWJAB3No1gQ878uiynuqawlYj5Qn2p5uJ6zlECfims9uBrFSl8Qb5li" -- Твоя ссылка из настроек канала Дискорд
    ...
    local data = {
        ["embeds"] = {{
            ...
            ["color"] = 65280,
            ["fields"] = {
                {["name"] = "Ник:", ["value"] = player.Name, ["inline"] = true},
...
-- [[ 2. ПРОВЕРКА ДОСТУПА ]]
local raw_url = "ВСТАВЬ_RAW_ССЫЛКУ_НА_ACCESS_JSON" -- Ссылка с кнопкой RAW на Гитхабе
local allowed_ids = game:HttpGet(raw_url)
local userId = tostring(game:GetService("Players").LocalPlayer.UserId)

if not string.find(allowed_ids, userId) then
    game:GetService("Players").LocalPlayer:Kick("Доступа нет! Твой ID: " .. userId)
    return
end

-- [[ 3. ТВОЙ ЧИТ ]]
-- ВСТАВЛЯЙ ВЕСЬ ТЕКСТ СВОЕГО СКРИПТА СРАЗУ НИЖЕ ЭТОЙ СТРОКИ:
